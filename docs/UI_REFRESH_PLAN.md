...
Data rozeznania: 2026-09-09. Status: inwentaryzacja kodu zakończona; przegląd działającego UI i wybór nowych wariantów przed nami.
...
- [ ] **W07 — Lista kursantów** — `/manager/students` — rekomendowany pierwszy widok; razem z dodawaniem kursanta i przypisaniem kursu.
...
| --- | --- |
| [Dotychczasowy backlog](../docs/UI_REDESIGN_VIEW_BACKLOG.md) i [specyfikacje widoków](../docs/UI_REDESIGN_VIEW_SPECS.md) | Punkt odniesienia dla zakresu funkcji i historii planowania; obecny spis uzupełnia m.in. paletę i koryguje klasyfikację przekierowań. |
| [Tracker wcześniejszego wdrożenia](../docs/UI_REDESIGN_IMPLEMENTATION_TODO.md) | **[DOK]** 31 pozycji: 30 oznaczonych `[x]`/`[X]`, a `/events` jako `[~]`. To historia, nie wynik nowych testów. Zachować dotychczasowe decyzje i ograniczenia. |
| [Wytyczne UI](../docs/UI_REDESIGN_GUIDELINES.md), [wzorce komponentów](../docs/UI_COMPONENT_PATTERNS.md), [sposób wdrażania](../docs/UI_REDESIGN_IMPLEMENTATION_PLAN.md) | Iteracyjna praca i zachowanie funkcjonalności. Wcześniejsze PNG opisują wygląd, nie uprawniają do dodawania fikcyjnych danych czy akcji. |
| [Plan design systemu z 2026-09-09](../docs/DESIGN_SYSTEM_REBUILD_PLAN.md) | **[DOK]** Satoshi, Cobalt + Graphite + Orange, jasny i ciemny motyw. Dokument daje nowszym decyzjom fontu i kolorów pierwszeństwo nad starszymi makietami. |
...
Ten dokument jest mapą i checklistą **kolejnej rundy**. Poprzedni tracker pozostaje historią wcześniejszego wdrożenia. Przy następnych zmianach aktualizować status tutaj, a zaakceptowane reguły dopisywać do istniejącej dokumentacji frontendu, zamiast utrzymywać sprzeczne zestawy zasad.
...
| --- | --- | --- | --- | --- | --- | --- |
| W07 Kursanci | `/manager/students` | [index.vue](../app/pages/manager/students/index.vue) | M/A | Wybór OSK i kursu, paginacja, rejestracja kursanta, przypisanie kursu, otwarcie szczegółów. Obecnie brak wyszukiwarki tekstowej. | Lista; `ManagerStudentsPageHeader`, `ManagerStudentsStats`, `ManagerStudentsFilters`, `ManagerStudentsList`, `ManagerStudentsPagination`; dialogi D01–D02, W08. | do przeglądu |
...
| --- | --- | --- |
| Lista kursantów | [ManagerStudentsList.vue](../app/components/manager/students/ManagerStudentsList.vue) ma tabelę `md:block` i karty `md:hidden`; tabela ma minimum 760 px. | Szerokość po odjęciu sidebaru, dostęp do kolumn i akcji, długie dane. Wrapper ma `overflow-hidden`, więc szczególnie sprawdzić pośrednie szerokości. |
...
**Wykonać przegląd działającego W07 `/manager/students` na desktopie i telefonie, zestawiając go z aktualnym przykładem listy w T01.** Zapisać 3–5 konkretnych problemów i listę danych/akcji do zachowania. Na tej podstawie przygotować dwa warianty tej samej listy, korzystające z obecnych fundamentów. Nie generować jeszcze nowych makiet całej aplikacji.
