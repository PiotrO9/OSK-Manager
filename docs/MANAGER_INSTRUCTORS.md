...
| `/manager/instructors/:id` | [`app/pages/manager/instructors/[id].vue`](../app/pages/manager/instructors/[id].vue)   | Szczegóły; edycja w modalu [`ManagerInstructorEditDialog`](../app/components/manager/instructors/ManagerInstructorEditDialog.vue) (bez zamykania po kliknięciu w tło) — PATCH przez BFF, `$fetch` przy wejściu / zmianie `id` |
...
- Przy ustawionym **upstream** (`resolveUpstreamBase`): proxy do backendu z nagłówkiem `Authorization: Bearer` z ciasteczka `access_token` — logika w [`server/utils/instructorsBff.ts`](../server/utils/instructorsBff.ts) (`bffUpstreamInstructorsList`, `bffUpstreamInstructorsGetById`, `bffUpstreamInstructorsPatch`).
- **Tryb mock** (bez upstreamu): po `requireManagerFromCookie` zwracana jest koperta `{ success: true, data: … }`; lista z [`mockInstructorsListPayload`](../server/utils/mockInstructorsList.ts), szczegół przez [`mockInstructorsGetById`](../server/utils/mockInstructorsList.ts), aktualizacja przez [`mockInstructorsPatchById`](../server/utils/mockInstructorsList.ts) (wyszukanie `id` po wszystkich szkołach w pamięciowym store).
